...
function Uninstall-TeamViewer {
    # Desinstala silenciosamente cualquier version existente de TeamViewer
    # Busca el UninstallString en el registro y lo ejecuta en modo silencioso
    $uninstallPaths = @(
        ...
        Get-ChildItem $basePath -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            if ($entry.DisplayName -like "*TeamViewer*") {
                ...
                if ($uninstStr) {
                    ...
                    try {
                        # Convertir el UninstallString a llamada silenciosa
                        if ($uninstStr -like "*.exe*") {
                            $exe  = ($uninstStr -split '"' | Where-Object { $_ -like "*.exe" })[0]
                            if (-not $exe) { $exe = $uninstStr.Split(" ")[0] }
                            Start-Process -FilePath $exe `
                                          -ArgumentList "/S /silent /uninstall" `
                                          -Wait -NoNewWindow `
                                          -ErrorAction Stop
                        }
                        Write-Log "  [OK] Version anterior desinstalada." "Green"
                    } catch {
                        Write-Log "  [WARN] No se pudo desinstalar automaticamente: $_" "Yellow"
                    }
                }
            }
        }
