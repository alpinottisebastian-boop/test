...
function Install-Apps {
    Write-Log "=== PASO 3: INSTALACION / ACTUALIZACION DE APLICACIONES ===" "Cyan"
    ...
    foreach ($app in $apps) {
        try {
            ...
            if (-not $isInstalled) {
                ...
                if ($LASTEXITCODE -eq 0) {
                    Write-Log "  [OK] $($app.Name) instalado correctamente." "Green"
                } else {
                    Write-Log "  [ERROR] $($app.Name) fallo al instalar (exit: $LASTEXITCODE)." "Red"
                    $script:ErrorCount++
                }

            } else {
                # --- UPGRADE ---
                Write-Log "  Actualizando: $($app.Name)..." "White"
                ...
                if ($LASTEXITCODE -eq 0) {
                    Write-Log "  [OK] $($app.Name) actualizado correctamente." "Green"
                    ...
                    if ($LASTEXITCODE -eq 0) {
                        Write-Log "  [OK] $($app.Name) reinstalado correctamente (--force)." "Green"
                    } else {
                        Write-Log "  [ERROR] $($app.Name) fallo incluso con --force (exit: $LASTEXITCODE)." "Red"
                        $script:ErrorCount++
                    }
                }
            }
            ...
            Write-Log "  [ERROR] Excepcion procesando $($app.Name): $_" "Red"
            $script:ErrorCount++
        }
    }
